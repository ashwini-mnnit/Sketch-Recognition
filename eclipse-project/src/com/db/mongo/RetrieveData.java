...
public class RetrieveData {
	 ...
	 * ) { Sketcher s = new Sketcher(); NickName name = new NickName();
	 * name.setNickname(sSketch.getAuthor());
	 * s.setNickname(sSketch.getAuthor()); sketcher.add(s); }
	 * 
	 * public void addArg(ArrayList<Arg> arg, List<SousaArg> argList) { for
	 * (SousaArg it : argList) { Arg a = new Arg(it.getType(),it.getId());
	 * arg.add(a); } }
	 * 
	 * public void addPoint(ArrayList<Point> point ,ArrayList<SousaStroke>
	 * sousaStrokes) { for (SousaStroke it : sousaStrokes) {
	 * ArrayList<SousaPoint> pointList = it.getPointList();
	 * 
	 * for (SousaPoint it1 : pointList) { Point p = new Point();
	 * p.setId(it1.getId()); p.setTime(Long.parseLong(it1.getTime())); Double d
	 * = it1.getxCoordinate(); p.setX(d.floatValue()); d = it1.getyCoordinate();
	 * p.setY(d.floatValue()); point.add(p); } } } public void
	 * addShape(ArrayList<Shape> shape, ArrayList<SousaStroke> sousaStrokes){
	 * for (SousaStroke it : sousaStrokes) { Shape s = new Shape();
	 * PremitiveStrokeType ptype = it.getPrimitiveType(); if (ptype != null) {
	 * s.setType(ptype.getName()); } ArrayList<Arg> arg = new ArrayList<Arg>();
	 * addArg(arg, it.getArgList()); s.setArg(arg); s.setId(it.getId());
	 * shape.add(s); } }
	 * 
	 * /*public Sketch getSketchMlObjectFromSouse(SousaSketch sSketch) { Sketch
	 * sketch = new Sketch(); sketch.setId(sSketch.getId());
	 * 
	 * ArrayList<Sketcher> sketcher= new ArrayList<Sketcher> ();
	 * addSketcher(sketcher,sSketch); sketch.setSketcher(sketcher);
	 * 
	 * ArrayList<Shape> shape= new ArrayList<Shape>();
	 * addShape(shape,sSketch.getSousaStrokes()); sketch.setShape(shape);
	 * 
	 * ArrayList<Point> point = new ArrayList<Point>(); addPoint(point,
	 * sSketch.getSousaStrokes()); sketch.setPoint(point); return sketch; }
	 */
	...
	public Sketch getSketchMlObject(MechanixSketch mSketch) {
		...
		for (MechanixShape it : mShape) {
			...
			/*
			 * if(mStroke.getDraw_color() != null) { float c =
			 * Float.parseFloat(mStroke.getDraw_color()); shape.setColor(c); }
			 */
			if (mStroke != null) {
